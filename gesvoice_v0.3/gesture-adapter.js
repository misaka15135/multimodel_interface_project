...
(function () {
  var fallbackOrigin = 'gesture-adapter-' + Math.random().toString(36).slice(2);
  var fallbackSeq = 0;
  ...
  window.__mmPublishGesture = function (gestureName, confidence) {
    ...
    if (window.MMFusion) {
      window.MMFusion.publish(event);
      return;
    }
    document.dispatchEvent(new CustomEvent('mmfusion:event', {
      detail: {
        source: event.source,
        type: event.type,
        action: event.action,
  ...
  window.__mmPublishPointer = function (x, y) {
    var value = { point: { x: x, y: y } };
    if (window.MMFusion) {
      window.MMFusion.setContext('pointerTarget', value);
      return;
    }
    document.dispatchEvent(new CustomEvent('mmfusion:context', {
      detail: { slot: 'pointerTarget', value: value, origin: fallbackOrigin },
    }));
  };

  console.log('[GestureAdapter] 就绪 —— 在手势触发处调用 __mmPublishGesture / __mmPublishPointer' +
    (window.MMFusion ? '' : ' (DOM fallback)'));
  // 接入步骤与映射详见 FUSION.md
})();
