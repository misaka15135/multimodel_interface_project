...
// gaze-engine.js 是追踪算法核心，既可以被 offscreen.js 使用，也方便以后复用到桌面版。
// 输入：HTMLVideoElement 摄像头画面。
// 输出：标准化坐标 point(x/y 均为 0~1)、眨眼点击事件、状态和调试指标。

// MediaPipe Face Landmarker 的关键点索引。
// 这里只取眼睛、鼻尖、额头、下巴等少量点，避免整套 478 点都混在业务逻辑里。
...
// 五点标定：中心、左、右、上、下。标定会记录这些方向上的原始头部数值。
...
function median(values) {
  // 中位数比平均值更抗抖动，适合短窗口过滤 landmark 噪声。
...
function faceBox(landmarks) {
  // 计算脸部包围盒，用来归一化头部转动幅度，减少远近距离对结果的影响。
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
...
function headMetrics(landmarks) {
  // 用鼻尖相对眼睛中心的位置估算头部朝向。
  // 水平：鼻尖相对左右眼中心偏移；垂直：鼻尖相对眼睛中心和脸高的偏移。
...
function waitForVideoReady(video) {
  // getUserMedia 成功后，video 元素仍需要一点时间拿到第一帧。
  // MediaPipe 必须等 videoWidth/videoHeight 有效后才能稳定 detectForVideo。
...
export class GazeEngine {
  constructor(options = {}) {
    // 默认配置偏保守：头部移动不会太灵敏，眨眼点击也尽量过滤自然眨眼。
    this.config = {
      smoothing: 0.08,
      speed: 0.9,
      deadZone: 0.04,
    ...
    this.lastBlinkAt = 0;
    this.calibration = {
      // profile 会保存 center/left/right/up/down 五个方向的原始数值。
      active: false,
      step: 0,
      samples: [],
      profile: null
  ...
  async start(video) {
    // 启动顺序很重要：
    // 先申请摄像头并确认 video 有画面，再加载模型，最后进入预测循环。
    // 这样用户能更快看到“摄像头是否已授权”的状态。
    this.video = video;
    if (!this.mediaStream) {
    ...
    if (!this.faceLandmarker) {
      // MediaPipe runtime、wasm、task 模型都放在扩展本地目录，不依赖网络。
  ...
  startCalibration() {
    // 标定开始后，每一帧都会收集当前方向样本，样本够了自动进入下一个目标点。
    this.calibration = {
      active: true,
      step: 0,
      samples: [],
  ...
  normalizeWithCalibration(rawX, rawY) {
    // 将原始头部/眼睛数值转换成屏幕标准化坐标。
    // 未完成标定时使用 fallback gain；标定完成后使用用户自己的头部活动范围。
  ...
  scaleResponse(point) {
    // responseScale 是“头部灵敏度”。小于 1 会让移动更迟钝、更稳。
  ...
  stabilizeRaw(rawX, rawY) {
    // 保存最近几帧原始值并取中位数，降低 landmark 抖动导致的指示点飘动。
  ...
  processLandmarks(landmarks) {
    // 单帧主处理：从 landmarks 得到头部方向、眨眼状态、标准化坐标，并输出给外部。
    ...
    if (eyeOpen < this.config.minEyeOpen) {
      // 眨眼时暂停坐标更新，避免闭眼造成 landmark 瞬间漂移。
      this.onMetrics({
        rawX,
        rawY,
        eyeRawX,
  ...
  processBlink(eyeOpen) {
    // 防误触策略：
    // long 模式要求闭眼时间超过 longBlinkMs；
    // double 模式要求两次自然短眨眼在 doubleBlinkWindowMs 内发生。
  ...
  emitBlinkClick(kind, duration, eyeOpen) {
    // 输出点击事件，真正的 DOM click 由 service worker 转给 content-script 执行。
  ...
  predict() {
    // 预测循环。这里使用 setTimeout 而不是 requestAnimationFrame，
    // 因为隐藏 offscreen 页面中 rAF 可能被 Chrome 降频，导致后台更新不稳定。
