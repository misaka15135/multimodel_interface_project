...
(function (exports) {
  ...
  /** 通知外部监听器（向后兼容 v0.3 的 onEvent API） */
  ...
  function bindEvents() {
    ...
    // 多模态融合：订阅其他模态事件（v0.4 仅观察，不重复执行手势自身的 DOM 操作）
    if (fusion) {
  ...
  const controller = {
    ...
    async init() {
      ...
      console.log('[VoiceExt] Controller v0.4 initialized —',
  ...
  controller.init().then(() => {
    ...
    console.log('[VoiceExt] 声控助手 v0.4 已就绪 — 点击右下角按钮开始');
  });
