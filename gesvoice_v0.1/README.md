# GesVoice Fusion Test v0.1

这是一个无摄像头也能跑的语音 + 手势融合测试扩展。

## 目的

- 验证 MAIN world 的手势侧能通过 `MMFusion` 发事件给 isolated world 的语音侧。
- 验证手势侧写入 `pointerTarget` 后，语音执行器可以执行 `click_target`。
- 在真实摄像头不可用时，用页面左下角的模拟器先测融合链路。

## 使用

1. Chrome/Edge 打开 `chrome://extensions/`。
2. 开启开发者模式。
3. 加载已解压扩展，选择 `gesvoice_v0.1` 文件夹。
4. 打开任意普通网页。
5. 用左下角 `GesVoice v0.1` 面板点击“自动测试”。
